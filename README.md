...
Using the version with the integrated serial adapter: You will have to remove the sensor and connect the GND Pin with the WRT (write) Pin via a jumper. After that you can upload the code to it. Remove the jumper and plug the board out then in again to reset it.  
